...
# the matching simulator, and puts the PNGs it attached into the artifacts directory (issue #46).
#
...
# xcodebuild refuses to write into a result bundle that exists, and a bundle left by an earlier run
# would be exported as if it were this one's.
rm -rf "${RESULT_BUNDLE}"
...
SIMULATOR_UDID="$(SCRIPT_QUIET=1 SIM_DEVICE_TYPE="${DEVICE_TYPE}" sim-boot --number "${SCREENSHOT_SIMULATOR_NUMBER}" | sed -n 's/^DEVICE_UDID=//p' | tail -n 1)"
...
echo "== running ${SCREENSHOT_SCHEME} on ${DEVICE_TYPE}"
xcodebuild \
