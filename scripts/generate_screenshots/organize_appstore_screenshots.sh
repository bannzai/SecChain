#!/bin/bash
# Takes the pages out of the capture runs' result bundles and names them the way the rest of the
# pipeline expects: artifacts/<device class>/<language>/<device class>-<page>.png (issue #46).
#
# Kept apart from the capture so that a naming change can be re-applied to result bundles that are
...
for language in "${SCREENSHOT_LANGUAGES[@]}"; do
  ...
  EXPORT_DIRECTORY="${SCREENSHOT_WORK_DIRECTORY}/${DEVICE}-${language}-attachments"
  ...
  echo "== exporting the attached pages of ${DEVICE} in ${language}"
  xcrun xcresulttool export attachments --path "${RESULT_BUNDLE}" --output-path "${EXPORT_DIRECTORY}" > /dev/null

  python3 - "${EXPORT_DIRECTORY}" "${SCREENSHOT_ARTIFACTS_DIRECTORY}" "${DEVICE}" "${language}" "${SCREENSHOT_PAGE_COUNT}" <<'PY'
import json
import pathlib
import re
import shutil
...
for test in manifest:
    for attachment in test.get("attachments", []):
        ...
        if matched is None or matched.group(1) != language or matched.group(2) != device:
            continue
        destination = artifacts_directory / device / language
        destination.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(export_directory / attachment["exportedFileName"], destination / f"{device}-{matched.group(3)}.png")
        exported += 1

print(f"organized {exported} page(s) of {device} in {language} under {artifacts_directory}")
if exported != expected:
    sys.exit(f"Expected {expected} pages of {device} in {language} in the result bundle, found {exported}")
PY
done
