...
| The Mac reads a `DevicePairing` of another version (`secchain pair`) | Pairing fails with the error. **Pair Again** on the iPhone creates a new key, publishes it in the iPhone's version, and withdraws the old record; every Mac then has to run `secchain pair` again, because the old key can no longer sign. A Mac that is already paired keeps its own copy of the key, which carries no version, so the version change alone does not unpair it — only pairing the iPhone again does |
